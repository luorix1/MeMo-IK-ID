...
``best_model.pt`` is produced by ``ik_id/trainV2.py`` and contains everything needed
besides the ONNX file for correct input scaling.
"""
...
def ik_indices_unilateral_paired(input_indices: List[int]) -> Tuple[int, int]:
    ...
    if len(input_indices) != 2 or len(input_indices) % 2 != 0:
        raise ValueError(f"Expected paired input_indices length 2 for sagittal_knee, got {input_indices}")
