...
class CascadeHip(BaseController):
    ...
    def step(self, s: Sensors) -> CtrlResult:
        ...
        seq_r = self.x_r.push_last(x_last_r)
        seq_l = self.x_l.push_last(x_last_l)

        assist_gate_r = self.gate_r.update(seq_r[1])
        assist_gate_l = self.gate_l.update(seq_l[1])

        x_batch = np.stack([seq_r, seq_l], axis=0).astype(np.float32)
        ...
        return CtrlResult(
            ...
            extra={
                ...
                "model_in_angle_raw_r": float(enc_r_raw),
                "model_in_angle_raw_l": float(enc_l_raw),
                "model_in_vel_raw_r": float(vel_r_raw),
                "model_in_vel_raw_l": float(vel_l_raw),
