...
class Mocap_trigger:
    ...
    def start_client(self):
        ...
        try:
            print("[CONNECTING] Connecting to mocap server...")
            self.client.connect((self.server_ip, self.port_number))
            print(f"[CONNECTED] {self.server_ip}:{self.port_number}")
        except ConnectionRefusedError:
            print(f"[ERROR] Cannot connect to {self.server_ip}:{self.port_number}")
