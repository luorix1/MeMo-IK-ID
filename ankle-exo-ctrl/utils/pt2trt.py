"""
pt2trt.py  —  PyTorch → ONNX → TensorRT conversion for ankle-exo-ctrl TCN models.
...
Usage (from ankle-exo-ctrl root):
...
Examples:
    ...
    # Jetson: CPU ONNX export + small TRT workspace (avoids OOM)
    python utils/pt2trt.py --pt best_model.pt --fp16 --cpu --workspace-mb 64

    # Rebuild TRT from an already-exported ONNX (no torch GPU needed)
    python utils/pt2trt.py --onnx best_model.onnx --trt best_model.trt --fp16 --workspace-mb 64
"""
...
# ---------------------------------------------------------------------------
# Locate model.py: it lives one directory above ankle-exo-ctrl (os_kinetics).
# ---------------------------------------------------------------------------
_SCRIPT_DIR  = os.path.dirname(os.path.abspath(__file__))       # .../ankle-exo-ctrl/utils
_CTRL_ROOT   = os.path.dirname(_SCRIPT_DIR)                      # .../ankle-exo-ctrl
_PARENT_ROOT = os.path.dirname(_CTRL_ROOT)                       # .../os_kinetics (has model.py)
...
def _pick_torch_device(prefer_cuda: bool = True) -> torch.device:
    """Prefer CUDA when usable; fall back to CPU for ONNX export.

    Pip-installed torch often targets a newer CUDA than the Jetson driver
    (RuntimeError: driver too old). ONNX export does not need GPU — TensorRT
    builds the engine from the ONNX file using the system CUDA/TRT stack.
    """
    if not prefer_cuda:
        print("[INFO] Using CPU for ONNX export (--cpu).")
        return torch.device("cpu")
    if not torch.cuda.is_available():
        print("[WARN] torch.cuda.is_available() is False — using CPU for ONNX export.")
        return torch.device("cpu")
    try:
        torch.empty(1, device="cuda")
        print("[INFO] Using CUDA for ONNX export.")
        return torch.device("cuda")
    except Exception as e:
        print(
            f"[WARN] CUDA unusable with this PyTorch build ({e}). "
            "Falling back to CPU for ONNX export; TensorRT build is unchanged."
        )
        return torch.device("cpu")
...
def pt_to_trt(
    ...
    # ------------------------------------------------------------------
    device = _pick_torch_device(prefer_cuda=prefer_cuda)
    tcn_kw = _tcn_ctor_kwargs(model_config)
    tcn = TCN(**tcn_kw).eval()
    tcn.load_state_dict(ckpt["model_state_dict"])
    model = _TCNLastStep(tcn).eval().to(device)
    ...
    dummy_input = torch.randn(*in_shape, device=device)
    ...
    # Free torch GPU memory before TRT build (if any).
    del model, tcn, dummy_input
    if device.type == "cuda":
        try:
    ...
    onnx_to_trt(
        ...
        io_hint=f"{in_shape} → (1, {n_output_channels})",
    )
...
def onnx_to_trt(
    ...
    """Build a TensorRT engine from an ONNX file (no PyTorch CUDA required)."""
    if not os.path.isfile(onnx_path):
        raise FileNotFoundError(f"ONNX file not found: {onnx_path}")

    workspace_bytes = max(8, int(workspace_mb)) * (1 << 20)
    print(f"[INFO] TRT workspace limit: {workspace_mb} MiB")
    ...
    if serialized_engine is None:
        raise RuntimeError(
            "TensorRT engine build failed (often GPU OOM on Jetson). "
            "Try: free GPU memory (close browsers/other CUDA apps), reboot, then:\n"
            "  python utils/pt2trt.py --onnx best_model.onnx --fp16 --workspace-mb 64\n"
            "or FP32:\n"
            "  python utils/pt2trt.py --onnx best_model.onnx --workspace-mb 64"
        )

    os.makedirs(os.path.dirname(os.path.abspath(trt_engine_path)) or ".", exist_ok=True)
    with open(trt_engine_path, "wb") as f:
        f.write(serialized_engine)

    print(f"[SUCCESS] TRT engine saved : {trt_engine_path}")
    if io_hint:
        print(f"          Engine I/O       : {io_hint}")
    else:
        print(f"          Engine I/O       : {tuple(net_in.shape)} → {tuple(net_out.shape)}")
...
def parse_args() -> argparse.Namespace:
    default_cfg = os.path.join(_CTRL_ROOT, "cfg", "final.yaml")

    p = argparse.ArgumentParser(
        description="Convert an os_kinetics TCN .pt checkpoint to a TensorRT engine.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--pt", default=None, help="Path to the input .pt checkpoint.")
    p.add_argument(
        "--onnx",
        default=None,
        help="Build TRT from an existing ONNX (skips PyTorch export). "
             "If omitted with --pt, writes <pt>.onnx next to the checkpoint.",
    )
    p.add_argument(
        "--trt", default=None,
        help="Output .trt path. Defaults to <pt|onnx path> with .trt extension.",
    )
    p.add_argument(
    ...
    p.add_argument(
        "--cpu",
        action="store_true",
        help="Force CPU for ONNX export (use when pip torch CUDA mismatches the driver).",
    )
    p.add_argument(
        "--workspace-mb",
        type=int,
        default=256,
        help="TensorRT builder workspace limit in MiB. Lower if you hit GPU OOM "
             "(try 128 or 64 on Jetson).",
    )
    return p.parse_args()
...
def main() -> None:
    ...
    if args.pt is None and args.onnx is None:
        raise SystemExit("Provide --pt and/or --onnx.")

    if args.onnx is not None and args.pt is None:
        onnx_path = os.path.abspath(args.onnx)
        trt_path = (
            os.path.abspath(args.trt)
            if args.trt
            else onnx_path.replace(".onnx", ".trt")
        )
        onnx_to_trt(
            onnx_path=onnx_path,
