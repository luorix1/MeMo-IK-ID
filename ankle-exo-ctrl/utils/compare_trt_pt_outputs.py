...
Supports sample windows taken from a logged ankle-exo .npz file.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        choices=["enc", "model"],
        default="enc",
        help="enc: ankle_vel_{r,l}; model: model_in_ankle_vel_raw_{r,l}",
    )
...
def build_samples(
    ...
    # Prefer model-input channels when present (sign-corrected).
    angle_key = f"model_in_ankle_angle_raw_{side_sfx}"
    if angle_key in data.files:
        angle = np.asarray(data[angle_key], dtype=np.float32)
    else:
        angle = np.asarray(data[f"ankle_pos_{side_LR}"], dtype=np.float32)

    if vel_key == "model" and f"model_in_ankle_vel_raw_{side_sfx}" in data.files:
        vel = np.asarray(data[f"model_in_ankle_vel_raw_{side_sfx}"], dtype=np.float32)
    elif f"ankle_vel_{side_LR}" in data.files:
        vel = np.asarray(data[f"ankle_vel_{side_LR}"], dtype=np.float32)
    else:
        vel = np.asarray(data[f"model_in_ankle_vel_raw_{side_sfx}"], dtype=np.float32)
